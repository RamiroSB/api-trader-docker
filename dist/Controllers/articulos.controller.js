...
exports.GetArticulos = void 0;
const database_1 = require("../database/database");
function GetArticulos(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        let sql_articulos = `SELECT
       ...
       AND art.Bundle <> '1'`;
        ...
        if (sku) {
            ...
            const articulos = yield conn.query(sql_articulos, [sku]);
            // @ts-ignore
            let art = [];
            // @ts-ignore
            ...
            if (art[0].Imagen) {
                // @ts-ignore
                let Position = art[0].Imagen.lastIndexOf('\\') + 1;
                // @ts-ignore
            ...
            const articulos = yield conn.query(sql_articulos, [marca]);
            // @ts-ignore
            let xmarcas = [];
            // @ts-ignore
            ...
            for (let i = 0; i < xmarcas.length; i++) {
                ...
                if (xmarcas[i].Imagen) {
                    // @ts-ignore
                    let Position = xmarcas[i].Imagen.lastIndexOf('\\') + 1;
                    // @ts-ignore
            ...
            const articulos = yield conn.query(sql_articulos);
            // @ts-ignore
            let xarticulos = [];
            // @ts-ignore
            ...
            for (let i = 0; i < xarticulos.length; i++) {
                ...
                if (xarticulos[i].Imagen) {
                    // @ts-ignore
                    let Position = xarticulos[i].Imagen.lastIndexOf('\\') + 1;
                    // @ts-ignore
